    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        if len(numeric_responses) >= 2:
            ...
            plot_index_rows.append({
                "Response": "all_numeric_responses",
                "Plot": "correlation_heatmap",
                "File": str(corr_path),
                "Status": "saved",
            })
            correlation_saved = True

        for response in response_cols:
            df_sub = df[[mainplot, subplot, response]].dropna().copy()
            if df_sub.empty:
                continue
            ...
            if "Residual" in anova_table.index:
                ...
                mse_error = anova_table.loc["Residual", "sum_sq"] / df_error if df_error else np.nan
            else:
                df_error = anova_table.iloc[-1]["df"]
                mse_error = anova_table.iloc[-1]["sum_sq"] / df_error if df_error else np.nan
            ...
            n_eff = means["n"].min() if not means.empty else np.nan
            alpha = 0.05
            t_critical = t.ppf(1 - alpha / 2, df_error) if pd.notna(df_error) and df_error > 0 else np.nan
            lsd = t_critical * np.sqrt(2 * mse_error / n_eff) if pd.notna(t_critical) and pd.notna(mse_error) and pd.notna(n_eff) and n_eff > 0 else np.nan

            letters_dict = get_lsd_letters_cld(means, lsd) if pd.notna(lsd) else {}
            ...
            plot_guide = pd.DataFrame({"Recommended Plot": recommend_plots(df_sub, response, [mainplot, subplot])})
            response_summary = pd.DataFrame({
                ...
                "Value": [
                    response,
                    means["group"].nunique(),
                    means["Mean"].min() if not means.empty else np.nan,
                    means["Mean"].max() if not means.empty else np.nan,
                    anova_table.loc[f"C({mainplot})", "P-value"] if f"C({mainplot})" in anova_table.index else np.nan,
                    anova_table.loc[f"C({subplot})", "P-value"] if f"C({subplot})" in anova_table.index else np.nan,
                    anova_table.loc[f"C({mainplot}):C({subplot})", "P-value"] if f"C({mainplot}):C({subplot})" in anova_table.index else np.nan,
                    assumptions.loc[assumptions["Check"] == "Shapiro-Wilk residual normality", "Result"].iloc[0],
                    assumptions.loc[assumptions["Check"] == "Levene homogeneity of variance", "Result"].iloc[0],
                ],
            })
            ...
            profile_path = assets_dir / f"{response_slug}_profile.png"
            diagnostics_path = assets_dir / f"{response_slug}_diagnostics.png"
            histogram_path = assets_dir / f"{response_slug}_histogram.png"

            histogram_requested = "histogram" in plot_guide["Recommended Plot"].tolist()

            saved_plots = [
                ("group_boxplot", create_group_boxplot(df_sub, response, "group", group_boxplot_path)),
                ("interaction_plot", create_interaction_plot(df_sub, response, mainplot, subplot, interaction_path)),
                ("mean_profile_plot", create_mean_profile_plot(df_sub, response, mainplot, subplot, profile_path)),
            ...
            run_rows.append({
                "Response": response,
                "MainPlot_Pvalue": anova_table.loc[f"C({mainplot})", "P-value"] if f"C({mainplot})" in anova_table.index else np.nan,
                "SubPlot_Pvalue": anova_table.loc[f"C({subplot})", "P-value"] if f"C({subplot})" in anova_table.index else np.nan,
                "Interaction_Pvalue": anova_table.loc[f"C({mainplot}):C({subplot})", "P-value"] if f"C({mainplot}):C({subplot})" in anova_table.index else np.nan,
                "Residual_Normality": assumptions.loc[assumptions["Check"] == "Shapiro-Wilk residual normality", "Result"].iloc[0],
                "Variance_Homogeneity": assumptions.loc[assumptions["Check"] == "Levene homogeneity of variance", "Result"].iloc[0],
                "LSD": lsd,
            })

        if plot_index_rows:
            pd.DataFrame(plot_index_rows).to_excel(writer, sheet_name="Plot_Index", index=False)
        elif not correlation_saved:
            pd.DataFrame(columns=["Response", "Plot", "File", "Status"]).to_excel(writer, sheet_name="Plot_Index", index=False)

        if run_rows:
            pd.DataFrame(run_rows).to_excel(writer, sheet_name="Run_Summary", index=False)

    print(f"Split-Plot ANOVA results saved to Excel: {Path(output_path)}")
