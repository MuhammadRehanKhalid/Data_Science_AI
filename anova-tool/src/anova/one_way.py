...
def one_way_anova(df, output_path):
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, factor_cols)})
            response_summary = pd.DataFrame({
                'Metric': ['response', 'groups', 'mean_min', 'mean_max', 'anova_pvalue', 'residual_normality', 'variance_homogeneity'],
                'Value': [
                    response,
                    means['group'].nunique(),
                    means['Mean'].min() if not means.empty else np.nan,
                    means['Mean'].max() if not means.empty else np.nan,
                    anova_table['P-value'].iloc[0] if len(anova_table) else np.nan,
            ...
            pd.DataFrame({'Metric': ['alpha', 't_critical', 'df_error', 'mse_error', 'lsd'], 'Value': [alpha, t_critical, df_error, mse_error, lsd]}).to_excel(writer, sheet_name=f'{base}_LSD', index=False)
            means.to_excel(writer, sheet_name=f'{base}_Means', index=False)
            assumptions.to_excel(writer, sheet_name=f'{base}_Checks', index=False)
            response_summary.to_excel(writer, sheet_name=f'{base}_Summary', index=False)
            plot_guide.to_excel(writer, sheet_name=f'{base}_Plots', index=False)
            if not posthoc.empty:
                posthoc.to_excel(writer, sheet_name=f'{base}_Posthoc', index=False)
                build_posthoc_guide_table(posthoc).to_excel(writer, sheet_name=f'{base}_Posthoc_Guide', index=False)
            ...
            run_rows.append({
                'Response': response,
                'ANOVA_Pvalue': anova_table['P-value'].iloc[0] if len(anova_table) else np.nan,
    ...
    print(f'One-way ANOVA results saved to Excel: {Path(output_path)}')
