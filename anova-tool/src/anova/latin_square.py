...
def latin_square_anova(df, row, column, treatment, output_path):
    response_cols = df.columns.difference([row, column, treatment]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[row, column, treatment, response]].dropna().copy()
            if df_sub.empty:
                continue

            formula = f'{response} ~ C({row}) + C({column}) + C({treatment})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [row, column, treatment])})
    ...
    print(f'Latin Square ANOVA results saved to Excel: {Path(output_path)}')
