...
def rcbd_anova(df, treatment, block, output_path):
    response_cols = df.columns.difference([treatment, block]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[treatment, block, response]].dropna().copy()
            if df_sub.empty:
                continue
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [treatment, block])})
    ...
    print(f'RCBD ANOVA with advanced outputs saved: {Path(output_path)}')
