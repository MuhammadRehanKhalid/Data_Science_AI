...
def two_way_anova(df, factor1, factor2, output_path):
    response_cols = df.columns.difference([factor1, factor2]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[factor1, factor2, response]].dropna().copy()
            if df_sub.empty:
                continue
            df_sub['group'] = df_sub[[factor1, factor2]].astype(str).agg('_'.join, axis=1)
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [factor1, factor2])})
            ...
            saved_plots = [
                ...
                ('mean_profile_plot', create_mean_profile_plot(df_sub, response, factor1, factor2, profile_path)),
    ...
    print(f'Two-way ANOVA results saved to Excel: {Path(output_path)}')
