...
def split_plot_latin_square_anova(df, row, column, mainplot, subplot, output_path):
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        for response in response_cols:
            df_sub = df[[row, column, mainplot, subplot, response]].dropna()

            # Group for LSD letters
            df_sub['group'] = df_sub[[mainplot, subplot]].astype(str).agg('_'.join, axis=1)

            # Latin Square structure + Split-Plot effects
    ...
    print("✅ Split-Plot Latin Square ANOVA results saved to Excel.")
