...
def split_plot_latin_square_anova(df, row, column, mainplot, subplot, output_path):
    response_cols = df.columns.difference([row, column, mainplot, subplot]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[row, column, mainplot, subplot, response]].dropna().copy()
            if df_sub.empty:
                continue

            df_sub['group'] = df_sub[[mainplot, subplot]].astype(str).agg('_'.join, axis=1)
            formula = f'{response} ~ C({row}) + C({column}) + C({mainplot}) * C({subplot})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [row, column, mainplot, subplot])})
    ...
    print(f'Split-Plot Latin Square ANOVA results saved to Excel: {Path(output_path)}')
