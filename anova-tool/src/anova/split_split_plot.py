...
def split_split_plot_anova(df, mainplot, subplot, subsubplot, output_path):
    response_cols = df.columns.difference([mainplot, subplot, subsubplot]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[mainplot, subplot, subsubplot, response]].dropna().copy()
            if df_sub.empty:
                continue

            df_sub['group'] = df_sub[[mainplot, subplot, subsubplot]].astype(str).agg('_'.join, axis=1)
            formula = f'{response} ~ C({mainplot}) * C({subplot}) * C({subsubplot})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [mainplot, subplot, subsubplot])})
    ...
    print(f'Split-Split Plot ANOVA results saved to Excel: {Path(output_path)}')
