...
def strip_split_anova(df, strip1, strip2, subplot, output_path):
    response_cols = df.columns.difference([strip1, strip2, subplot]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[strip1, strip2, subplot, response]].dropna().copy()
            if df_sub.empty:
                continue

            df_sub['group'] = df_sub[[strip1, strip2, subplot]].astype(str).agg('_'.join, axis=1)
            formula = f'{response} ~ C({strip1}) * C({strip2}) * C({subplot})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [strip1, strip2, subplot])})
    ...
    print(f'Strip-Split ANOVA results saved to Excel: {Path(output_path)}')
