...
def four_way_anova(df, factor1, factor2, factor3, factor4, output_path):
    response_cols = df.columns.difference([factor1, factor2, factor3, factor4]).tolist()
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[factor1, factor2, factor3, factor4, response]].dropna().copy()
            if df_sub.empty:
                continue

            df_sub['group'] = df_sub[[factor1, factor2, factor3, factor4]].astype(str).agg('_'.join, axis=1)
            formula = f'{response} ~ C({factor1})*C({factor2})*C({factor3})*C({factor4})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [factor1, factor2, factor3, factor4])})
    ...
    print(f'Exported all Four-Way ANOVA results successfully: {Path(output_path)}')
