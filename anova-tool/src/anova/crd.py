...
def crd_anova(df, output_path):
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        for response in response_cols:
            df_sub = df[[treatment_col, response]].dropna().copy()
            if df_sub.empty:
                continue

            df_sub['group'] = df_sub[treatment_col].astype(str)
            formula = f'{response} ~ C({treatment_col})'
            ...
            plot_guide = pd.DataFrame({'Recommended Plot': recommend_plots(df_sub, response, [treatment_col])})
    ...
    print(f'CRD ANOVA results saved to Excel: {Path(output_path)}')
